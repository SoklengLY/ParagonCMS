<template>
  <div class="flex flex-col w-1/2 items-center">
    <h1
      class="text-3xl font-bold text-blue-900 text-center pt-4 pb-6 h-fit w-1/2">
      Headers
    </h1>
    <div class="flex gap-5 my-auto w-3/4 flex-col" v-for="header in headers" :key="header.id">
        <div class="flex justify-between m-1 p-2 px-5 shadow items-center rounded">
            ...
            <div class="flex gap-5 my-auto">
              <NuxtLink :to="`/admin/buildheader?id=${header.id}`" class="my-auto text-blue-900 text-lg flex items-center">
              ...
              <v-menu>
                ...
                <v-list>
                  <v-list-item @click="toggleHeader(header.id)">
                    <v-list-item-title>{{ header.isActive ? 'Unpublish' : 'Publish' }}</v-list-item-title>
                  </v-list-item>
